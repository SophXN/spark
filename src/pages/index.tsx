...
const HomePage: React.FC<Props> = () => {
  ...
  const { loading, error } = useManageCompanyAndLocations(sessionData?.user.companyId as string, sessionData?.user.id as string, { enabled: !!sessionData });

  const [homePageEventData, yourEventsData] = api.useQueries((t) => [t.events.getHomePageEvents("", { enabled: !!sessionData }), t.events.getYourEvents(sessionData?.user.companyId as string, { enabled: !!sessionData })])
    ...
    <div>
      ...
      <main>
        <div className="mx-auto max-w-7xl px-3">
          ...
          <div className="w-full max-w-7xl overflow-x-auto whitespace-nowrap hide-scrollbar">
            {loadingYourEventData ? (
              <div className="flex gap-2">
                <Skeleton className="w-1/3 h-[400px] rounded-md" />
                <Skeleton className="w-1/3 h-[400px] rounded-md" />
                <Skeleton className="w-1/3 h-[400px] rounded-md" />
              </div>
            ) : (
              <div className="flex gap-2">
                {yourEventsData.data?.map((event) => (
                  <div
                    ...
                    className="inline-block min-w-[400px] w-1/3"
                  >
                    <EventCard
                      key={event.eventId}
